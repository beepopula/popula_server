module.exports = function (app) {
  ...
  app.get('/api/v1/user/getNotifications', async (ctx, next) => {
    ...
    let User = ctx.model("user")
    let q = {"$or": [{accountId: accountId, "$or": [{type: "comment"}, {type: "post"}]},{"commentContent.accountId": accountId, "$or": [{type: "comment"}, {type: "post"}, {type: "mainPost"}]}, {'options.At': accountId}]}
    if (lastTime) {
